...
import { X, Trophy, Medal, Crown, Sparkles } from 'lucide-react';
...
export const ClassDetailModal: React.FC<Props> = ({ isOpen, onClose, classNameStr, students }) => {
  ...
  const topStudents = classStudents.slice(0, 5); // Top 5
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/80 backdrop-blur-sm p-4 animate-in zoom-in-95 duration-200">
      <div className="bg-slate-900 border border-amber-500/30 rounded-2xl w-full max-w-2xl shadow-2xl overflow-hidden flex flex-col max-h-[85vh]">
        ...
        {/* Header */}
        <div className="relative bg-gradient-to-r from-slate-900 via-slate-800 to-slate-900 p-6 text-center border-b border-slate-700">
          <button 
             onClick={onClose} 
          ...
          <h2 className="text-3xl font-black text-transparent bg-clip-text bg-gradient-to-b from-amber-200 to-amber-600 mb-1">
            {classNameStr}
          </h2>
        ...
        <div className="p-6 overflow-y-auto bg-slate-900/50">
           <div className="flex items-center gap-2 mb-4">
              ...
              <h3 className="text-lg font-bold text-white">המובילים בכיתה</h3>
           </div>
